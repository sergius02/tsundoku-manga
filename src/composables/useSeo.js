...
export function useSeo(options = {}) {
  function updateMeta() {
    ...
    const ogTitle =
      document.querySelector('meta[property="og:title"]') || createMeta('property', 'og:title')
    ...
    const ogType =
      document.querySelector('meta[property="og:type"]') || createMeta('property', 'og:type')
    ogType.content = 'website'

    const ogUrl =
      document.querySelector('meta[property="og:url"]') || createMeta('property', 'og:url')
    ogUrl.content = options.canonical || window.location.href

    if (options.ogImage) {
      const ogImage =
        document.querySelector('meta[property="og:image"]') || createMeta('property', 'og:image')
      ogImage.content = options.ogImage
    }

    const twitterCard =
      document.querySelector('meta[name="twitter:card"]') || createMeta('name', 'twitter:card')
    twitterCard.content = 'summary_large_image'

    const twitterTitle =
      document.querySelector('meta[name="twitter:title"]') || createMeta('name', 'twitter:title')
