  ...
  try {
    ...
    const restoreTransaction = db.transaction(() => {
      ...
      for (const manga of mangas) {
        insertManga.run(
          manga.id, manga.title, manga.author, manga.publisher,
          manga.cover_url, manga.notes, manga.created_at, manga.updated_at
        );
      }
      ...
      for (const volume of volumes) {
        insertVolume.run(
          volume.id, volume.manga_id, volume.isbn, volume.title,
          volume.volume_number, volume.status, volume.acquired ? 1 : 0, volume.cover_url
        );
      }
      ...
      db.prepare('INSERT OR IGNORE INTO api_config (api_name, enabled) VALUES (?, ?)').run('openlibrary', 1);
      db.prepare('INSERT OR IGNORE INTO api_config (api_name, enabled) VALUES (?, ?)').run('googlebooks', 1);
    });
